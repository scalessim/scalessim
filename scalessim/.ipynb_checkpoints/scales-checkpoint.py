...
class SCALES:
    ...
    def cube(self,targ=None,psf=None,cube_in=None,
        ...
        self.skybg = SkyBG(1,1)
        self.skytrans = SkyTrans(1,1)
        if skytransoff==True: self.skytrans.y = np.ones(self.skytrans.y.shape)*u.dimensionless_unscaled
        ...
        if targ!=None:
            if len(psf.shape)>3:
                cube_seq = []
                for iii in range(len(psf)):
                    if ((type(vapor) is np.ndarray) or (type(vapor) is list)):
                        vapor_in = vapor[iii]
                    else: vapor_in = vapor
                    if ((type(airmass) is np.ndarray) or (type(airmass) is list)):
                        airmass_in = airmass[iii]
                    else: airmass_in = airmass
                    #print(iii,vapor_in,type(airmass),airmass_in)
                    self.fp.SkyBG = SkyBG(vapor_in,airmass_in)
                    self.fp.SkyTrans = SkyTrans(vapor_in,airmass_in)
                ...
                self.fp.SkyBG = SkyBG(vapor,airmass)
                self.fp.SkyTrans = SkyTrans(vapor,airmass)
